...
const DENY_FOR_PUBLISH = new Set(['.git', '.ghennai', '_ghennai', 'uploads', 'node_modules', 'dist', 'light-assets', 'assets_old', 'sites', 'light-site', 'README.md', 'config.json', '.gitignore'])
...
async function findSiteDir(ws, preferred) {
  ...
  // الطلب الصريح للجذر ('.') أو عدم وجود تفضيل: ننشر الجذر إن كان يحوي index.html
  const rootIdx = path.join(ws, 'index.html')
  if (fs.existsSync(rootIdx)) return ws
  // لا يوجد موقع في الجذر — نبحث عن أحدث مجلد فرعي يحوي index.html (التوافق مع القديم)
  let best = ws
  let bestT = 0
  try {
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    // 1) مجلد الموقع: المجلد المطلوب صراحةً (لا تخمين بأحدث mtime)
    ...
    // 1.5) صورة نظيفة للنشر (فقط ملفات الموقع الحقيقية)
    ...
    const gitInit = await run(`git init -b main && git add -A && git -c user.email="${host}@ghennai.local" -c user.name="${name}" commit -m "Generated by Ghennai" --allow-empty`, stageDir)
    ...
    // 3) مستودع ورابط لكل موقع: الجذر → الريبو الموجود (mouh-website)، أي مشروع مجلد → ريبو خاص به ثابت
    ...
    if (!fs.existsSync(readme)) {
      ...
      await run(`git add -A && git -c user.email="${host}@ghennai.local" -c user.name="${name}" commit -m "docs: readme" --allow-empty`, stageDir)
